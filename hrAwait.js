...
(async function () {
    try {
        let browserWillbeLauncedPromise = await puppeteer.launch({
            headless: false,  //browser is visible
            defaultViewport: null,   //FullScreen browser
            args: ['--start-maximized'],
        })

        let newTab = await browserWillbeLauncedPromise.newPage();
        await newTab.goto(loginLink)
        await newTab.type('input[id="input-1"]', email, { delay: 40 })
        await newTab.type('input[id="input-2"]', password, { delay: 100 })
        await newTab.click('button[data-analytics="LoginPassword"]', { delay: 70 })
